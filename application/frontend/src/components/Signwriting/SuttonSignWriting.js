...
export var ssw = {
  ...
  pair: function (value) {
    ...
    if (value && str.length == 2) {
      return [
        str.charCodeAt(0).toString(16).toUpperCase(),
        str.charCodeAt(1).toString(16).toUpperCase()
      ]
    } else {
      return false
    }
  ...
  fsw2swu: function (text) {
    ...
    fsw = fsw.replace(
      /B!/g,
      String.fromCharCode(
        0xd800 + ((code + 1 - 0x10000) >> 10),
        0xdc00 + ((code + 1 - 0x10000) & 0x3ff)
      )
    )
    fsw = fsw.replace(
      /L/g,
      String.fromCharCode(
        0xd800 + ((code + 2 - 0x10000) >> 10),
        0xdc00 + ((code + 2 - 0x10000) & 0x3ff)
      )
    )
    fsw = fsw.replace(
      /M/g,
      String.fromCharCode(
        0xd800 + ((code + 3 - 0x10000) >> 10),
        0xdc00 + ((code + 3 - 0x10000) & 0x3ff)
      )
    )
    fsw = fsw.replace(
      /R/g,
      String.fromCharCode(
        0xd800 + ((code + 4 - 0x10000) >> 10),
        0xdc00 + ((code + 4 - 0x10000) & 0x3ff)
      )
    )
  ...
  random: function (type, swu) {
    ...
    var start = range[0]
    var end = range[1]
    var rBase = Math.floor(
      Math.random() * (parseInt(end, 16) - parseInt(start, 16) + 1) + parseInt(start, 16)
    )
  ...
  colorize: function (key) {
    ...
    if (ssw.is(key, 'movement')) {
      color = 'CC0000'
    }
    if (ssw.is(key, 'dynamic')) {
      color = 'FF0099'
    }
    if (ssw.is(key, 'head')) {
      color = '006600'
    ...
    if (ssw.is(key, 'location')) {
      color = '884411'
    }
    if (ssw.is(key, 'punctuation')) {
      color = 'FF9900'
    }
    return color
  },
  ...
  size: function (text) {
    ...
    if (
      'S16d03 S16d0f S16d1f S16d2f S16d23 S16d43 S16d53 S1710d S1711d S1712d S17311 S17321 S17733 S1773f S17743 S1774f S17753 S1775f S16d33 S1713d S1714d S17301 S17329 S1715d'.indexOf(
        key
      ) > -1
    ) {
      width = '20'
  ...
  svg: function (text, options) {
    ...
    if (!options.line) {
      ...
      options.line = /^[0-9a-fA-F]{3}([0-9a-fA-F]{3})?$/g.test(options.line)
        ? '#' + options.line
        : options.line
    }
    if (!options.fill) {
      ...
      options.fill = /^[0-9a-fA-F]{3}([0-9a-fA-F]{3})?$/g.test(options.fill)
        ? '#' + options.fill
        : options.fill
    }
    if (!options.back) {
      ...
      options.back = /^[0-9a-fA-F]{3}([0-9a-fA-F]{3})?$/g.test(options.back)
        ? '#' + options.back
        : options.back
    }
    options.E = []
    ...
    if (styling) {
      ...
      if (rs) {
        ...
        if (colors[0]) {
          options.line = /^[0-9a-fA-F]{3}([0-9a-fA-F]{3})?$/g.test(colors[0])
            ? '#' + colors[0]
            : colors[0]
        }
        if (colors[1]) {
          options.fill = /^[0-9a-fA-F]{3}([0-9a-fA-F]{3})?$/g.test(colors[1])
            ? '#' + colors[1]
            : colors[1]
        }
      }
      ...
      rs = stylings[2].match(
        /D[0-9]{2}_([0-9a-f]{3}([0-9a-f]{3})?|[a-wyzA-Z]+)(,([0-9a-f]{3}([0-9a-f]{3})?|[a-wyzA-Z]+))?_/g
      )
      if (rs) {
        for (i = 0; i < rs.length; i += 1) {
          ...
          if (colors[0]) {
            colors[0] = /^[0-9a-fA-F]{3}([0-9a-fA-F]{3})?$/g.test(colors[0])
              ? '#' + colors[0]
              : colors[0]
          }
          if (colors[1]) {
            colors[1] = /^[0-9a-fA-F]{3}([0-9a-fA-F]{3})?$/g.test(colors[1])
              ? '#' + colors[1]
              : colors[1]
          }
    ...
    for (i = 0; i < syms.length; i += 1) {
      ...
      if (!options.css) {
        gelem +=
          "style=\"pointer-events:none;font-family:'SuttonSignWritingFill';font-size:" +
          (options.F[i + 1] ? 30 * options.F[i + 1][0] : 30) +
          'px;fill:' +
          (options.E[i + 1]
            ? options.E[i + 1][1]
              ? options.E[i + 1][1]
      ...
      if (!options.css) {
        ...
        gelem +=
          "font-family:'SuttonSignWritingLine';font-size:" +
          (options.F[i + 1] ? 30 * options.F[i + 1][0] : 30) +
          'px;fill:' +
          (options.E[i + 1]
            ? options.E[i + 1][0]
    ...
    if (options.laned) {
      svg =
        '<div style="padding:10px;position:relative;width:' +
        w +
        'px;height:' +
        h +
        'px;left:' +
  ...
  canvas: function (text, options) {
    ...
    for (i = 0; i < len; i += 1) {
      ...
      context.font =
        (options.F[i + 1] ? 30 * options.size * options.F[i + 1][0] : 30 * options.size) +
        "px 'SuttonSignWritingFill'"
      context.fillStyle = options.E[i + 1]
        ? options.E[i + 1][1]
          ? options.E[i + 1][1]
          : options.fill
        : options.fill
    ...
    if (ssw.sign(swu)) {
      if (/^[Aa]?([Ss]L?)?$/.test(flags)) {
        ...
        if (flags.indexOf('A') > -1 || flags.indexOf('a') > -1) {
          //exact symbols or general symbols in order
          matches = swu.match(
            new RegExp(ssw.re['swu']['sort'] + '(' + ssw.re['swu']['symbol'] + ')*', 'g')
          )
          if (matches) {
            matched = matches[0]
            if (flags.indexOf('A') > -1) {
